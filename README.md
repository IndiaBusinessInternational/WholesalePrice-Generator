...
**Version:** `v2.6` (shown top-left). Use `v2.x` for patches, `v3` for big features.

## What's new
- **v2.6 — Manual entry (no bill).** For local purchases without a tax invoice, click **Enter Manually (No Bill)** in step 1 to open the entry grid directly. Type product name, HSN, GST %, Unit Price and Qty (leave **Amount** blank), add transport in step 3, then calculate. Tick **Incl. GST?** if the price you paid already includes GST (or there's none); untick to add GST on top of the rate.
